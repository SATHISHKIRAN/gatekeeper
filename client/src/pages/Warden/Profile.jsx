...
import { User, Shield, Key, Bell, Home, CheckCircle, Activity, Mail, Phone, Calendar } from 'lucide-react';
...
const WardenProfile = () => {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                const response = await axios.get('/api/warden/profile', {
                ...
                >
                    <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl shadow-xl border border-white/20 dark:border-slate-800 p-8 text-center relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                            <div className="w-32 h-32 mx-auto bg-gradient-to-br from-teal-100 to-emerald-100 dark:from-slate-800 dark:to-slate-800 rounded-full flex items-center justify-center text-4xl font-bold text-teal-600 dark:text-teal-400 mb-6 shadow-inner ring-4 ring-white dark:ring-slate-800">
                            ...
                            <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-teal-50 dark:bg-teal-900/20 text-teal-600 dark:text-teal-400 rounded-full text-xs font-bold uppercase tracking-wide border border-teal-100 dark:border-teal-800">
                                <Activity className="w-3 h-3" /> Active
                            </div>
                        </div>

                        {/* Quick Stats Row */}
                        <div className="mt-8 grid grid-cols-1 gap-4 border-t border-slate-100 dark:border-slate-800 pt-6">
                            <div className="text-center">
                                <p className="text-2xl font-bold text-slate-900 dark:text-white">{stats.occupancy}</p>
                                <p className="text-xs text-slate-400 uppercase tracking-wider font-semibold">Current Occupants</p>
                            </div>
                        </div>
                    </div>
                </motion.div>
                ...
                >
                    ...
                    <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl shadow-xl border border-white/20 dark:border-slate-800 p-8">
                        <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-2">
                            <Home className="w-5 h-5 text-teal-500" /> Information
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-slate-400 uppercase tracking-wide flex items-center gap-2">
                                    <Home className="w-3 h-3" /> Assigned Hostel
                                </label>
                                <p className="font-medium text-slate-900 dark:text-white truncate">{stats.hostel_name}</p>
                            </div>
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-slate-400 uppercase tracking-wide flex items-center gap-2">
                    ...
                    <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl shadow-xl border border-white/20 dark:border-slate-800 p-8">
                        <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-2">
                            <Key className="w-5 h-5 text-teal-500" /> Security
                        </h3>
