...
import { User, Shield, Key, Bell, Building, CheckCircle, Activity, Mail, Phone, Lock } from 'lucide-react';
...
const PrincipalProfile = () => {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                const response = await axios.get('/api/principal/profile', {
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-8 animate-fade-in p-6">
            <h1 className="text-3xl font-bold bg-gradient-to-r from-indigo-900 to-slate-800 dark:from-indigo-400 dark:to-slate-200 bg-clip-text text-transparent">
                Executive Profile
            </h1>
                ...
                >
                    <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl shadow-xl border border-white/20 dark:border-slate-800 p-8 text-center relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                            <div className="w-32 h-32 mx-auto bg-gradient-to-br from-indigo-100 to-purple-100 dark:from-slate-800 dark:to-slate-800 rounded-full flex items-center justify-center text-4xl font-bold text-indigo-600 dark:text-indigo-400 mb-6 shadow-inner ring-4 ring-white dark:ring-slate-800">
                                {userData.name?.charAt(0)}
                            </div>
                            <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-1">{userData.name}</h2>
                            <p className="text-sm text-slate-500 dark:text-slate-400 font-medium mb-4">{userData.role.toUpperCase()}</p>

                            <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 rounded-full text-xs font-bold uppercase tracking-wide border border-indigo-100 dark:border-indigo-800">
                                <Shield className="w-3 h-3" /> Principal
                            </div>
                        </div>

                        {/* Quick Stats Row */}
                        <div className="mt-8 grid grid-cols-3 gap-2 border-t border-slate-100 dark:border-slate-800 pt-6">
                            <div className="text-center">
                                <p className="text-xl font-bold text-slate-900 dark:text-white">{stats.total_students}</p>
                                <p className="text-[10px] text-slate-400 uppercase tracking-wider font-semibold">Students</p>
                            </div>
                            <div className="text-center">
                                <p className="text-xl font-bold text-slate-900 dark:text-white">{stats.total_staff}</p>
                                <p className="text-[10px] text-slate-400 uppercase tracking-wider font-semibold">Staff</p>
                            </div>
                            <div className="text-center">
                                <p className="text-xl font-bold text-slate-900 dark:text-white">{stats.active_passes}</p>
                                <p className="text-[10px] text-slate-400 uppercase tracking-wider font-semibold">Out Now</p>
                            </div>
                        </div>
                    </div>
                </motion.div>
                ...
                >
                    ...
                    <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl shadow-xl border border-white/20 dark:border-slate-800 p-8">
                        <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-2">
                            <Building className="w-5 h-5 text-indigo-500" /> Administrative Details
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-slate-400 uppercase tracking-wide flex items-center gap-2">
                                    <Lock className="w-3 h-3" /> Campus Security
                                </label>
                                <p className="font-medium text-emerald-600 dark:text-emerald-400">{stats.campus_status}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl shadow-xl border border-white/20 dark:border-slate-800 p-8">
                        <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-2">
                            <Key className="w-5 h-5 text-indigo-500" /> Security
                        </h3>
