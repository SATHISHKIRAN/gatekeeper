...
import { User, Shield, Key, Bell, Building, CheckCircle, Users, Activity, Mail, Phone, Calendar, School } from 'lucide-react';
...
const HODProfile = () => {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                const response = await axios.get('/api/hod/profile', {
                ...
                >
                    <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl shadow-xl border border-white/20 dark:border-slate-800 p-8 text-center relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                            <div className="w-32 h-32 mx-auto bg-gradient-to-br from-red-100 to-rose-100 dark:from-slate-800 dark:to-slate-800 rounded-full flex items-center justify-center text-4xl font-bold text-red-600 dark:text-red-400 mb-6 shadow-inner ring-4 ring-white dark:ring-slate-800">
                            ...
                            <div className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wide border ${stats.delegation_active ? 'bg-amber-50 text-amber-600 border-amber-100 dark:bg-amber-900/20 dark:text-amber-400 dark:border-amber-800' : 'bg-emerald-50 text-emerald-600 border-emerald-100 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-800'
                                }`}>
                                <Activity className="w-3 h-3" /> {stats.delegation_active ? 'Delegated' : 'On Duty'}
                            </div>
                        </div>
                        ...
                        <div className="mt-8 grid grid-cols-2 gap-4 border-t border-slate-100 dark:border-slate-800 pt-6">
                            <div className="text-center">
                                <p className="text-2xl font-bold text-slate-900 dark:text-white">{stats.staff_count}</p>
                                <p className="text-xs text-slate-400 uppercase tracking-wider font-semibold">Staff</p>
                            </div>
                            <div className="text-center">
                                <p className="text-2xl font-bold text-slate-900 dark:text-white">{stats.student_count}</p>
                                <p className="text-xs text-slate-400 uppercase tracking-wider font-semibold">Students</p>
                            </div>
                        </div>
                    </div>
                </motion.div>
                ...
                >
                    ...
                    <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl shadow-xl border border-white/20 dark:border-slate-800 p-8">
                        <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-2">
                            <School className="w-5 h-5 text-red-500" /> Department Details
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-slate-400 uppercase tracking-wide flex items-center gap-2">
                                    <Building className="w-3 h-3" /> Department Name
                                </label>
                                <p className="font-medium text-slate-900 dark:text-white truncate">{userData.department_name || 'N/A'}</p>
                            </div>
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-slate-400 uppercase tracking-wide flex items-center gap-2">
                                    <Activity className="w-3 h-3" /> Department Code
                                </label>
                                <p className="font-medium text-slate-900 dark:text-white">{userData.department_code || 'N/A'}</p>
                            </div>
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-slate-400 uppercase tracking-wide flex items-center gap-2">
                    ...
                    <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl shadow-xl border border-white/20 dark:border-slate-800 p-8">
                        <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-2">
                            <Key className="w-5 h-5 text-rose-500" /> Security
                        </h3>
