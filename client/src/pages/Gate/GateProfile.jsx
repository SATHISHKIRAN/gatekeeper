...
import { User, Shield, Key, Bell, Scan, QrCode, Activity, Mail, Phone, Lock } from 'lucide-react';
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                ...
                const response = await axios.get('/api/gate/profile', {
                ...
                >
                    <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl shadow-xl border border-white/20 dark:border-slate-800 p-8 text-center relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                            <div className="w-32 h-32 mx-auto bg-gradient-to-br from-orange-100 to-amber-100 dark:from-slate-800 dark:to-slate-800 rounded-full flex items-center justify-center text-4xl font-bold text-orange-600 dark:text-orange-400 mb-6 shadow-inner ring-4 ring-white dark:ring-slate-800">
                                {userData.name?.charAt(0)}
                            </div>
                            <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-1">{userData.name}</h2>
                            <p className="text-sm text-slate-500 dark:text-slate-400 font-medium mb-4">SECURITY OPERATIONS</p>

                            <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400 rounded-full text-xs font-bold uppercase tracking-wide border border-orange-100 dark:border-orange-800">
                                <Shield className="w-3 h-3" /> Gatekeeper
                            </div>
                        </div>
                        ...
                        <div className="mt-8 grid grid-cols-2 gap-4 border-t border-slate-100 dark:border-slate-800 pt-6">
                            <div className="text-center">
                                <p className="text-2xl font-bold text-slate-900 dark:text-white">{stats.scans_today || 0}</p>
                                <p className="text-xs text-slate-400 uppercase tracking-wider font-semibold">Scans Today</p>
                            </div>
                            <div className="text-center">
                                <p className="text-2xl font-bold text-emerald-500">{stats.status || 'Active'}</p>
                ...
                >
                    ...
                    <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl shadow-xl border border-white/20 dark:border-slate-800 p-8">
                        <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-2">
                            <Scan className="w-5 h-5 text-orange-500" /> Operational Details
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-slate-400 uppercase tracking-wide flex items-center gap-2">
                                    <QrCode className="w-3 h-3" /> Gate Assignment
                                </label>
                                <p className="font-medium text-slate-900 dark:text-white">{userData.assigned_gate || 'Main Gate'}</p>
                            </div>
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-slate-400 uppercase tracking-wide flex items-center gap-2">
                                    <Activity className="w-3 h-3" /> Shift Status
                                </label>
                                <p className="font-medium text-emerald-600 dark:text-emerald-400">On Duty</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl shadow-xl border border-white/20 dark:border-slate-800 p-8">
                        <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-2">
                            <Key className="w-5 h-5 text-orange-500" /> Security
                        </h3>
